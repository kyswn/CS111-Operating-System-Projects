...
int main(int argc, char **argv) {
  ...
  struct states state = {FAHRENHEIT, 1, 0, 0, 0, 0, 2, NULL, NULL, 0, 0, NULL};
  ...
  while (run_flag) {
	...
	if (buf == '\n') {
	  ...
	  if (strcmp(buffer + state.buf_head, "OFF\n") == 0) {
	    ...
	    dprintf(state.socket_fd, "%s SHUTDOWN\n", t_str_shutdown);
